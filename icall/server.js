...
function serverHandler(request, response) {
    ...
    try {
        ...
        var matched = false;
        ['/call/', '/dev/', '/dist/', '/socket.io/', '/node_modules/canvas-designer/', '/admin/'].forEach(function(item) {
        ...
        try {
            ...
            if (filename.search(/call/g) === -1 && filename.search(/admin/g) === -1 && stats.isDirectory() && config.homePage === '/call/index.html') {
        ...
        try {
            if (fs.statSync(filename).isDirectory()) {
                ...
                if (filename.indexOf(resolveURL('/call/MultiRTC/')) !== -1) {
                    filename = filename.replace(resolveURL('/call/MultiRTC/'), '');
                    filename += resolveURL('/call/MultiRTC/index.html');
                ...
                } else if (filename.indexOf(resolveURL('/call/dashboard/')) !== -1) {
                    filename = filename.replace(resolveURL('/call/dashboard/'), '');
                    filename += resolveURL('/call/dashboard/index.html');
                } else if (filename.indexOf(resolveURL('/call/video-conference/')) !== -1) {
                    filename = filename.replace(resolveURL('/call/video-conference/'), '');
                    filename += resolveURL('/call/video-conference/index.html');
                } else if (filename.indexOf(resolveURL('/call')) !== -1) {
                    filename = filename.replace(resolveURL('/call/'), '');
                    filename = filename.replace(resolveURL('/call'), '');
                    filename += resolveURL('/call/index.html');
                } else {
